...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt_file, prompt, images)
    {
        ...
        (Some(ExecCommand::Resume(args)), root_prompt_file, root_prompt, imgs) => {
            let prompt_arg = args
                ...
                .or(root_prompt);
            let prompt_text = resolve_prompt(prompt_arg, root_prompt_file);
        ...
        (None, root_prompt_file, root_prompt, imgs) => {
            let prompt_text = resolve_root_prompt(root_prompt, root_prompt_file);
            let mut items: Vec<UserInput> = imgs
                .into_iter()
                .map(|path| UserInput::LocalImage { path })
                .collect();
...
fn canceled_mcp_server_elicitation_response() -> Result<Value, String> {
    serde_json::to_value(xcodex_non_interactive::canceled_mcp_server_elicitation_response())
    ...
    let handle_result = match request {
        ...
        ServerRequest::CommandExecutionRequestApproval { request_id, params } => {
            reject_server_request(
                ...
                request_id,
                &method,
                xcodex_non_interactive::unsupported_exec_mode_thread_request(
        ...
        ServerRequest::ChatgptAuthTokensRefresh { request_id, .. } => {
            reject_server_request(
                ...
                request_id,
                &method,
                xcodex_non_interactive::unsupported_exec_mode_global_request(
...
fn resolve_root_prompt(prompt_arg: Option<String>, prompt_file: Option<PathBuf>) -> String {
    if prompt_file.is_some() {
        return resolve_prompt(prompt_arg, prompt_file);
    }
    ...
    match prompt_arg {
        ...
        maybe_dash => resolve_prompt(maybe_dash, None),
    }
}
