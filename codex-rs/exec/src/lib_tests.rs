...
fn resolves_large_prompt_from_stdin_when_piped() {
    ...
    let prompt = resolve_prompt_inner(None, None, || false, || Ok(payload.clone().into_bytes()))
        .expect("resolve");
...
#[test]
fn resolve_root_prompt_prefers_prompt_file_over_optional_stdin_append() {
    let mut tmp = tempfile::NamedTempFile::new().expect("temp file");
    write!(tmp, "prompt from file").expect("write prompt");

    let prompt = resolve_root_prompt(
...
#[test]
fn builds_custom_review_request_from_prompt_file() {
    let mut tmp = tempfile::NamedTempFile::new().expect("temp file");
    write!(tmp, "  custom review instructions from file  ").expect("write prompt");
    ...
    let args = ReviewArgs {
        ...
        commit: None,
        commit_title: None,
        prompt_file: Some(tmp.path().to_path_buf()),
        prompt: None,
    };
    let request = build_review_request(&args).expect("builds custom review request from file");

    let expected = ReviewRequest {
        target: ReviewTarget::Custom {
            instructions: "custom review instructions from file".to_string(),
        },
        user_facing_hint: None,
    };
