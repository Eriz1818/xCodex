...
pub enum Feature {
    ...
    Tui2,
    /// Deprecated compatibility flag. PowerShell UTF-8 handling no longer
    /// depends on a feature toggle.
    PowershellUtf8,
        ...
        for (k, v) in m {
            ...
            match feature_for_key(k) {
                Some(feat) => {
                    ...
                    if k != feat.key() || matches!(feat.stage(), Stage::Deprecated | Stage::Removed)
                    {
                        self.record_legacy_usage_force(k.as_str(), feat);
                    }
                    if *v {
                        self.enable(feat);
                    } else {
...
fn legacy_usage_notice(alias: &str, feature: Feature) -> (String, Option<String>) {
    ...
    match feature {
        ...
        Feature::PowershellUtf8 => (
            "`[features].powershell_utf8` is deprecated and has no effect.".to_string(),
            Some(
                "Remove it from config.toml. PowerShell UTF-8 behavior no longer depends on this feature flag."
                    .to_string(),
            ),
        ),
        _ => {
            ...
            if alias == canonical {
                let summary = match feature.stage() {
                    Stage::Deprecated => format!("`{label}` is deprecated."),
                    Stage::Removed => format!("`{label}` is deprecated and has no effect."),
                    Stage::UnderDevelopment | Stage::Experimental { .. } | Stage::Stable => {
                        format!("`{label}` is deprecated. Use `[features].{canonical}` instead.")
                    }
                };
                let details = match feature.stage() {
                    Stage::Deprecated | Stage::Removed => Some(
                        "Remove it from config.toml unless you still need temporary compatibility."
                            .to_string(),
                    ),
                    Stage::UnderDevelopment | Stage::Experimental { .. } | Stage::Stable => None,
                };
