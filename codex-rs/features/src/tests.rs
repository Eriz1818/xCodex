...
#[test]
fn removed_feature_key_records_deprecation_notice_for_canonical_usage() {
    let mut features = Features::with_defaults();
    features.apply_map(&BTreeMap::from([("powershell_utf8".to_string(), true)]));

    assert_eq!(features.enabled(Feature::PowershellUtf8), true);
    let usages = features.legacy_feature_usages().collect::<Vec<_>>();
    assert_eq!(usages.len(), 1);
    assert_eq!(usages[0].feature, Feature::PowershellUtf8);
    assert!(usages[0].summary.contains("has no effect"));
}
