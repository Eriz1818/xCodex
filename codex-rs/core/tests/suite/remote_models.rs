...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn remote_models_chatgpt_keeps_models_not_supported_in_api() -> Result<()> {
    skip_if_no_network!(Ok(()));
    skip_if_sandbox!(Ok(()));

    let server = MockServer::start().await;
    let slug = "remote-chatgpt-only";
    let mut remote_model = test_remote_model(slug, ModelVisibility::List, 0);
    remote_model.supported_in_api = false;
    mount_models_once(
        &server,
        ModelsResponse {
            models: vec![remote_model],
    ...
    let model = available
        .iter()
        .find(|model| model.model == slug)
        .expect("chatgpt auth should keep models not supported in API mode");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn remote_models_api_key_hides_models_not_supported_in_api() -> Result<()> {
    skip_if_no_network!(Ok(()));
    skip_if_sandbox!(Ok(()));

    let server = MockServer::start().await;
    let slug = "remote-api-hidden";
    let mut remote_model = test_remote_model(slug, ModelVisibility::List, 0);
    remote_model.supported_in_api = false;
    mount_models_once(
        &server,
    ...
    let available = manager
        ...
        .await;
    assert!(
        !available.iter().any(|model| model.model == slug),
        "api-key auth should hide models not supported in API mode"
    );
