...
impl Codex {
    ...
    async fn spawn_internal(args: CodexSpawnArgs) -> CodexResult<CodexSpawnOk> {
        ...
        let refresh_strategy = match (&session_source, &conversation_history) {
            (SessionSource::SubAgent(_), _) | (_, InitialHistory::Resumed(_)) => {
...
impl TurnContext {
    ...
    pub(crate) async fn with_model(&self, model: String, models_manager: &ModelsManager) -> Self {
        ...
        Self {
            ...
            worktree_workspace_root: self.worktree_workspace_root.clone(),
            worktrees_pinned_paths: self.worktrees_pinned_paths.clone(),
            ...
            exclusion_counters: Arc::clone(&self.exclusion_counters),
            sensitive_paths: SensitivePathPolicy::new_with_exclusion(
                self.cwd.clone().to_path_buf(),
                self.exclusion.clone(),
            ),
            unattested_output_policy: self.unattested_output_policy,
...
#[derive(Clone, Debug, PartialEq, Eq)]
struct ExclusionMatchSummary {
    reason: crate::content_gateway::RedactionReason,
    value: String,
    count: usize,
}

fn summarize_exclusion_matches(
    report: &crate::content_gateway::ScanReport,
) -> Vec<ExclusionMatchSummary> {
    let mut out: Vec<ExclusionMatchSummary> = Vec::new();
    ...
    for match_info in &report.matches {
        ...
        if let Some(idx) = seen.get(&key) {
            ...
            out.push(ExclusionMatchSummary {
...
fn exclusion_reason_label(reason: crate::content_gateway::RedactionReason) -> &'static str {
    match reason {
...
fn exclusion_short_sha256_hex(text: &str) -> String {
...
fn exclusion_truncate_match_value(mut value: String) -> String {
    if value.len() <= 200 {
        return value;
    }
...
fn exclusion_match_value_display(
    ...
    match summary.reason {
        ...
        _ => exclusion_truncate_match_value(summary.value.clone()),
    }
}
...
async fn prompt_for_exclusion_match_selection(
    sess: &Session,
...
async fn maybe_prompt_for_exclusion_redaction(
    ...
    match answer.as_str() {
        ...
        "Add to allowlist" => {
            let candidates: Vec<ExclusionMatchSummary> = match_summaries
                .iter()
                .filter(|summary| {
                    matches!(
                        summary.reason,
            ...
            let selected = if candidates.len() == 1 {
                ...
                let options = candidates
                    ...
                    .collect::<Vec<_>>();
                let answer = prompt_for_exclusion_match_selection(
                    sess,
                    turn_context,
            ...
            match selected.reason {
                crate::content_gateway::RedactionReason::IgnoredPath => Some(
                    ExclusionRedactionDecision::AddAllowlistLiteral(selected.value),
                ),
                _ => Some(ExclusionRedactionDecision::AddAllowlistRegex(
                    selected.value,
                )),
            }
        }
...
fn block_response_item(item: &mut ResponseItem) {
    match item {
        ...
        ResponseItem::CustomToolCallOutput { output, .. } => {
            *output = FunctionCallOutputPayload::from_text("[BLOCKED]".to_string());
        }
        _ => {}
    }
}
...
impl Session {
    ...
    fn make_turn_context(
        ...
        TurnContext {
            ...
            sensitive_paths: SensitivePathPolicy::new_with_exclusion(
                session_configuration.cwd.clone().to_path_buf(),
    ...
    async fn new(
        ...
        let (mcp_connection_manager, cancel_token) = McpConnectionManager::new(
            ...
            match config.mcp_servers_startup_mode {
    ...
    pub(crate) async fn update_token_usage_info(
        ...
        let full_model_context_window = turn_context.model_info.context_window;
        let warning_message = if let Some(token_usage) = token_usage {
            let mut state = self.state.lock().await;
            ...
            if let Some(context_window) = model_context_window.filter(|window| *window > 0) {
    ...
    async fn update_prompt_token_estimate(&self, turn_context: &TurnContext, total_tokens: i64) {
        {
            ...
            if info.full_model_context_window.is_none() {
                info.full_model_context_window = turn_context.model_info.context_window;
            }
        ...
        I: IntoIterator<Item = &'a ResponseItem>,
    {
        ...
        let router = ToolRouter::from_config(
            ...
            ToolRouterParams {
                ...
                discoverable_tools: Some(Vec::new()),
        ...
        let prompt = Prompt {
            ...
            base_instructions,
            personality: turn_context.config.personality,
        ...
        let should_exit = async {
            match sub.op.clone() {
                ...
                Op::TerminateUnifiedExecSession { process_id } => {
                    handlers::terminate_unified_exec_session(&sess, sub.id.clone(), process_id)
                        .await;
                    false
                }
    ...
    pub async fn mcp_retry(
        ...
        let sandbox_state = {
            ...
            SandboxState {
                ...
                codex_linux_sandbox_exe: config.codex_linux_sandbox_exe.clone(),
                sandbox_cwd: cfg.cwd.clone().to_path_buf(),
                use_legacy_landlock: !cfg
                    .original_config_do_not_use
                    .features
...
async fn spawn_review_thread(
    ...
    let review_turn_context = TurnContext {
        ...
        sensitive_paths: SensitivePathPolicy::new_with_exclusion(
            parent_turn_context.cwd.clone().to_path_buf(),
...
pub(crate) async fn run_turn(
    ...
    let pre_sampling_compacted = if sess.auto_compact_enabled().await
        && turn_context.model_info.auto_compact_token_limit() != Some(0)
    {
...
async fn run_auto_compact(
    ...
    let hook_reason = match reason {
        CompactionReason::ContextLimit | CompactionReason::ModelDownshift => "auto",
        CompactionReason::UserRequested => "manual",
    };
    sess.user_hooks().pre_compact(
        sess.conversation_id.to_string(),
        turn_context.cwd.display().to_string(),
        ...
        if u64::from(retries) < max_retries {
            ...
            let delay = match &err {
                CodexErr::Stream(_, requested_delay) => {
                    requested_delay.unwrap_or_else(|| backoff(retries.into()))
                }
                _ => backoff(retries.into()),
            };
            warn!(
...
pub(crate) async fn built_tools(
    ...
    if !requested_mcp_servers.is_empty() {
        let discovered_mcp_servers = all_mcp_tools
            .values()
...
async fn try_run_sampling_request(
    ...
    let prompt_to_send = if turn_context.exclusion.enabled
