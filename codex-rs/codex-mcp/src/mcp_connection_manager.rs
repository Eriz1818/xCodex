...
async fn update_manifest_cache(
    ...
    let cached_tools = tools
        ...
        .map(|tool| CachedTool {
            ...
            description: tool.tool.description.clone().map(Cow::into_owned),
...
pub struct McpConnectionManager {
    ...
    codex_apps_tools_cache_key: Option<CodexAppsToolsCacheKey>,
...
pub enum McpConnectionStartupMode {
    Eager,
    Lazy,
    Manual,
}
...
impl McpConnectionManager {
    ...
    pub fn new_uninitialized(
        ...
        Self {
            ...
            startup_mode: McpConnectionStartupMode::Eager,
        }
    }
    ...
    pub async fn new(
        ...
        let tool_plugin_provenance = Arc::new(tool_plugin_provenance.clone());
            ...
            .filter(|(_, cfg)| cfg.enabled)
        {
            ...
            if startup_mode == McpConnectionStartupMode::Eager {
                let _ = emit_update(
        ...
        let manager = Self {
            ...
            sandbox_state: Some(initial_sandbox_state),
            tx_event: Some(tx_event.clone()),
            autoload_attempts: Arc::new(Mutex::new(HashMap::new())),
            store_mode,
            auth_entries,
            codex_apps_tools_cache_key: Some(codex_apps_tools_cache_key),
            tool_plugin_provenance: (*tool_plugin_provenance).clone(),
            startup_mode,
        };
        if startup_mode == McpConnectionStartupMode::Eager {
            tokio::spawn(async move {
    ...
    async fn client_by_name(&self, name: &str) -> Result<ManagedClient> {
        self.ensure_server_ready(name, StartupTrigger::ToolCall)
            .await
    ...
    pub async fn retry_servers(
        ...
        self.auth_entries = auth_entries.clone();
        self.sandbox_state = Some(sandbox_state.clone());
        self.tx_event = Some(tx_event.clone());
        self.server_configs.extend(mcp_servers.clone());
        self.autoload_attempts.lock().await.clear();
        self.startup_mode = McpConnectionStartupMode::Eager;

        let Some(codex_home) = self.codex_home.clone() else {
            return;
        };
        let codex_apps_tools_cache_key =
            self.codex_apps_tools_cache_key
                .clone()
                .unwrap_or(CodexAppsToolsCacheKey {
                    account_id: None,
        ...
        for (server_name, cfg) in mcp_servers.into_iter().filter(|(_, cfg)| cfg.enabled) {
            ...
            if let Some(origin) = transport_origin(&cfg.transport) {
                self.server_origins.insert(server_name.clone(), origin);
            }

            let tx_event = tx_event.clone();
            let auth_entry = auth_entries.get(&server_name).cloned();
            let sandbox_state = sandbox_state.clone();
            let ready_clients = Arc::clone(&self.ready_clients);
            let manifest_cache = Arc::clone(&self.manifest_cache);
            let codex_home = codex_home.clone();
            let cfg_for_cache = cfg.clone();
            join_set.spawn(async move {
                let outcome = async_managed_client.client().await;
    ...
    async fn ensure_server_ready(
        ...
        if self.startup_mode == McpConnectionStartupMode::Manual
            && !matches!(trigger, StartupTrigger::ManualLoad)
        {
            return Err(anyhow!(
                "MCP server '{server_name}' is configured for manual startup"
            ));
        }

        let event_id = match trigger {
            StartupTrigger::AutoLoad => "mcp_autoload",
            StartupTrigger::ManualLoad => "mcp_load",
            StartupTrigger::ToolCall => "mcp_tool_call",
        };
    ...
    pub async fn list_all_tools(&self) -> HashMap<String, ToolInfo> {
        ...
        for (server_name, cached) in &manifest_cache.servers {
            ...
            let cached_tools = cached
                ...
                .collect::<Vec<_>>();
            tools.extend(filter_tools(cached_tools, &filter));
        }
        qualify_tools(tools)
    }
