...
const UPDATE_CHECK_ENABLED: bool = true;
...
const XCODEX_LATEST_RELEASE_URL: &str =
    "https://api.github.com/repos/Eriz1818/xcodex/releases/latest";
...
async fn check_for_update(version_file: &Path) -> anyhow::Result<()> {
    let latest_version = if codex_core::config::is_xcodex_invocation() {
        let ReleaseInfo {
            tag_name: latest_tag_name,
        } = create_client()
            .get(XCODEX_LATEST_RELEASE_URL)
            .send()
            .await?
            .error_for_status()?
            .json::<ReleaseInfo>()
        ...
        match crate::update_action::get_update_action() {
            Some(crate::update_action::UpdateAction::BrewUpgrade) => {
