...
/// Compatibility wrapper for older TUI callsites that only need success/error.
///
/// Keep the most recent lease alive so Linux clipboard ownership does not vanish
/// immediately for legacy callers that have not been migrated yet.
pub(crate) fn copy_text(text: String) -> Result<(), String> {
    let lease = copy_to_clipboard(&text)?;
    LEGACY_CLIPBOARD_LEASE.with(|slot| {
        *slot.borrow_mut() = lease;
    });
    Ok(())
}
...
std::thread_local! {
    static LEGACY_CLIPBOARD_LEASE: std::cell::RefCell<Option<ClipboardLease>> =
