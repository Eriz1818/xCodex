...
async fn make_chatwidget_manual(
    ...
    let stored_collaboration_mode = base_mode;
...
async fn final_message_separator_is_emitted_without_phase_markers() {
    ...
    let begin = begin_exec(&mut chat, "call-no-phase-sep", "echo ok");
    end_exec(&mut chat, begin, "ok\n", "", 0);
    let cells = drain_insert_history(&mut rx);
    assert_eq!(cells.len(), 1, "expected finalized exec cell to flush");
    ...
    assert_eq!(
        cells.len(),
        2,
        "expected separator immediately before final message without phase markers"
    );
    let separator = lines_to_single_string(&cells[0]);
    assert!(
        separator.trim_start().starts_with('─'),
