...
fn diff_render_style_context_for_highlight(diff_highlight: bool) -> DiffRenderStyleContext {
    let mut context = current_diff_render_style_context();
    if !diff_highlight {
        context.color_level = DiffColorLevel::Ansi16;
        context.diff_backgrounds = ResolvedDiffBackgrounds::default();
    }
    context
}
...
fn render_line_count_summary(
    ...
    spans.push(RtSpan::styled(
        format!("+{added}"),
        base.patch(xcodex_style_add(diff_highlight)),
    ));
    spans.push(RtSpan::styled(" ", base));
    spans.push(RtSpan::styled(
        format!("-{removed}"),
        base.patch(xcodex_style_del(diff_highlight)),
    ));
    spans.push(RtSpan::styled(")", base));
    spans
}
...
fn render_changes_block(
    ...
    for (idx, r) in rows.into_iter().enumerate() {
        ...
        let lang = guess_lang_for_path(lang_path);
        let mut lines = vec![];
        render_change(
            &r.change,
            &mut lines,
...
fn render_change(
    ...
    let style_context = diff_render_style_context_for_highlight(diff_highlight);
    match change {
        FileChange::Add { content } => {
            // Pre-highlight the entire file content as a whole.
            let syntax_lines = use_syntax
                .then(|| highlight_lang.and_then(|l| highlight_code_to_styled_spans(content, l)))
                .flatten();
...
fn push_wrapped_diff_line(
    ...
    let (sign_char, sign_style) = match kind {
        DiffLineType::Insert => (
            '+',
            diff_sign_style(
                xcodex_style_add(diff_highlight),
                syntax_highlight,
                diff_highlight,
            ),
        ),
        DiffLineType::Delete => (
            '-',
            diff_sign_style(
                xcodex_style_del(diff_highlight),
                syntax_highlight,
                diff_highlight,
            ),
        ),
    ...
    let content_style = match kind {
        DiffLineType::Insert => {
            diff_content_style(xcodex_style_add(diff_highlight), syntax_highlight)
        }
        DiffLineType::Delete => {
            diff_content_style(xcodex_style_del(diff_highlight), syntax_highlight)
        }
        DiffLineType::Context => style_context(surface),
    };
    let gutter_style = diff_gutter_style(kind, syntax_highlight, diff_highlight, surface);
    let available_content_cols = width.saturating_sub(prefix_cols + 1).max(1);
    ...
    if !syntax_highlight {
        ...
        return wrapped_chunks
            ...
            .map(|(index, chunk)| {
                ...
                spans.extend(chunk);
                RtLine::from(spans).style(style_context(surface))
            })
            .collect();
    }
    ...
    for (line_idx, line) in highlighted.iter().enumerate() {
        let wrapped = word_wrap_line(line, RtOptions::new(available_content_cols));
        for (wrapped_idx, chunk) in wrapped.iter().enumerate() {
            ...
            if chunk.spans.is_empty() {
                spans.push(RtSpan::styled(String::new(), content_style));
            } else {
            ...
            out.push(RtLine::from(spans).style(style_context(surface)));
        }
    }

    if out.is_empty() {
        out.push(RtLine::from(vec![
            RtSpan::styled(format!("{ln_str:>gutter_width$} "), gutter_style),
...
fn xcodex_style_add(diff_highlight: bool) -> Style {
    if diff_highlight {
...
fn xcodex_style_del(diff_highlight: bool) -> Style {
    if diff_highlight {
...
fn diff_gutter_style(
    ...
    if syntax_highlight && !diff_highlight {
        match kind {
            DiffLineType::Insert => xcodex_style_add(false).add_modifier(Modifier::BOLD),
            DiffLineType::Delete => xcodex_style_del(false).add_modifier(Modifier::BOLD),
