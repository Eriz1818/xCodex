...
"#;
    ...
    fn tsx_highlight_config() -> &'static HighlightConfiguration {
        TSX_HIGHLIGHT_CONFIG.get_or_init(|| {
            ...
            #[expect(clippy::expect_used)]
            let mut config = HighlightConfiguration::new(language, "tsx", &query, "", "")
                .expect("load tsx query");
            config.configure(GENERIC_HIGHLIGHT_NAMES);
            config
        })
    }
    ...
    pub(crate) fn highlight_code_to_lines(code: &str, lang: &str) -> Vec<Line<'static>> {
        highlight_code_block_to_lines(Some(lang), code)
    }
    ...
    pub(crate) fn highlight_code_to_styled_spans(
        ...
        if !supports_highlighting(lang) {
            return None;
        }
        let lines = highlight_code_to_lines(code, lang);
        Some(lines.into_iter().map(|line| line.spans).collect())
    }
    #[cfg(test)]
    mod tests {
        use super::*;
...
fn upstream_highlight_code_to_lines(code: &str, lang: &str) -> Vec<Line<'static>> {
...
fn upstream_highlight_bash_to_lines(script: &str) -> Vec<Line<'static>> {
...
fn upstream_highlight_code_to_styled_spans(
    code: &str,
    lang: &str,
) -> Option<Vec<Vec<Span<'static>>>> {
...
pub(crate) fn supports_highlighting(lang: &str) -> bool {
    ...
    xcodex_tree_sitter::supports_highlighting(trimmed) || find_syntax(trimmed).is_some()
}
...
pub(crate) fn highlight_code_block_to_lines(lang: Option<&str>, code: &str) -> Vec<Line<'static>> {
    ...
    if xcodex_tree_sitter::supports_highlighting(lang) {
        return xcodex_tree_sitter::highlight_code_block_to_lines(Some(lang), code);
    }

    upstream_highlight_code_to_lines(code, lang)
}
...
pub(crate) fn highlight_code_to_lines(code: &str, lang: &str) -> Vec<Line<'static>> {
    ...
    if xcodex_tree_sitter::supports_highlighting(lang) {
        return xcodex_tree_sitter::highlight_code_to_lines(code, lang);
    }
...
pub(crate) fn highlight_bash_to_lines(script: &str) -> Vec<Line<'static>> {
    ...
    xcodex_tree_sitter::highlight_bash_to_lines(script)
}
...
pub(crate) fn highlight_bash_with_heredoc_overrides(script: &str) -> Vec<Line<'static>> {
    ...
    xcodex_tree_sitter::highlight_bash_with_heredoc_overrides(script)
}
...
pub(crate) fn highlight_code_to_styled_spans(
    ...
    if xcodex_tree_sitter::supports_highlighting(lang) {
        return xcodex_tree_sitter::highlight_code_to_styled_spans(code, lang);
    }

    upstream_highlight_code_to_styled_spans(code, lang)
}
