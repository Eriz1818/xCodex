...
impl ChatWidget {
    ...
    fn update_task_running_state(&mut self) {
        self.bottom_pane.set_task_running(
            self.agent_turn_running
                || self.mcp_startup_state.status().is_some()
    ...
    fn finalize_completed_assistant_message(&mut self, message: Option<&str>) {
        ...
        if self.suppress_legacy_agent_messages > 0 {
            ...
            self.log_separator_state("finalize_completed_assistant_message_suppressed", None);
        } else if self.stream_controller.is_none()
            && let Some(message) = message
            && !message.is_empty()
        {
            self.log_separator_state("finalize_completed_assistant_message_before", None);
    ...
    fn on_mcp_startup_update(&mut self, ev: McpStartupUpdateEvent) {
        let header = self.mcp_startup_state.on_update(ev.clone());
    ...
    fn update_mcp_startup_status(
                ...
                .all(|state| !matches!(state, McpStartupStatus::Starting))
        {
            let ready = current
                ...
                .filter_map(|(name, state)| {
                    matches!(state, McpStartupStatus::Ready).then_some(name.clone())
                })
                .collect::<Vec<_>>();
        ...
        if let Some(current) = &self.mcp_startup_status {
            ...
            let mut starting: Vec<_> = current
                .iter()
                .filter_map(|(name, state)| {
                    matches!(state, McpStartupStatus::Starting).then_some(name)
                })
                .collect();
            starting.sort();
    ...
    fn can_retry_mcp_startup_in_place(&self) -> bool {
    ...
    fn apply_mcp_startup_completion_state(
        &mut self,
        ready: Vec<String>,
        failed: Vec<String>,
        cancelled: Vec<String>,
    ) {
        let outcome = self.mcp_startup_state.on_complete(
            ...
            self.can_retry_mcp_startup_in_place(),
        );
    ...
    pub(crate) fn capture_thread_input_state(&self) -> Option<ThreadInputState> {
        ...
        Some(ThreadInputState {
            ...
            current_collaboration_mode: self.stored_collaboration_mode.clone(),
    ...
    pub(crate) fn restore_thread_input_state(&mut self, input_state: Option<ThreadInputState>) {
        ...
        if let Some(input_state) = input_state {
            self.stored_collaboration_mode = input_state.current_collaboration_mode;
    ...
    fn new_with_op_target(common: ChatWidgetInit, codex_op_target: CodexOpTarget) -> Self {
        ...
        widget.bottom_pane.set_footer_status_options(
            widget.config.tui_status_bar_show_model,
            widget.config.tui_status_bar_show_git_branch,
            widget.config.tui_status_bar_show_worktree,
        );
        widget
            .bottom_pane
            .set_footer_model_name(Some(widget.model_display_name().to_string()));
    ...
    fn queue_user_message(&mut self, user_message: UserMessage) {
        ...
        if !(user_message.text.trim().is_empty() && user_message.local_images.is_empty()) {
            self.set_mcp_startup_banner(None);
    ...
    fn is_local_slash_command(&self, user_message: &UserMessage) -> bool {
        if !user_message.local_images.is_empty() || user_message.text.lines().count() != 1 {
            return false;
        }
    ...
    fn submit_user_message(&mut self, user_message: UserMessage) {
        let is_local_slash_command = self.is_local_slash_command(&user_message);
        if !self.is_session_configured() && !is_local_slash_command {
    ...
    pub(crate) fn open_status_menu_view(&mut self, tab: crate::bottom_pane::StatusMenuTab) {
        ...
        let view = crate::bottom_pane::StatusMenuView::new(
            ...
            self.app_event_tx.clone(),
            status_cell,
            self.config.tui_status_bar_show_model,
    ...
    pub(crate) fn set_footer_status_options(
        ...
        self.config.tui_status_bar_show_model = show_model;
        self.config.tui_status_bar_show_git_branch = show_git_branch;
        self.config.tui_status_bar_show_worktree = show_worktree;
        self.bottom_pane
            .set_footer_status_options(show_model, show_git_branch, show_worktree);
    ...
    pub(crate) fn set_feature_enabled(&mut self, feature: Feature, enabled: bool) -> bool {
        self.config.features.set_enabled(feature, enabled);
    ...
    fn plan_mode_mask_with_overrides(&self) -> Option<CollaborationModeMask> {
        let mut mask = collaboration_modes::plan_mask(self.model_catalog.as_ref())?;
    ...
    pub(crate) fn effective_reasoning_effort(&self) -> Option<ReasoningEffortConfig> {
        self.effective_collaboration_mode().reasoning_effort()
    }
    ...
    fn refresh_model_display(&mut self) {
        ...
        self.bottom_pane
            .set_footer_model_name(Some(self.model_display_name().to_string()));
    ...
    ///
    /// Call this at the end of any setter that mutates `stored_collaboration_mode`,
    ...
    fn toggle_plan_mode(&mut self) {
        ...
        let next_mask = if target_kind == ModeKind::Plan {
            ...
            collaboration_modes::mask_for_kind(self.model_catalog.as_ref(), target_kind)
        };
    ...
    fn should_prompt_plan_mode_reasoning_scope(
        ...
        selected_effort != self.effective_reasoning_effort()
            || selected_model != self.stored_collaboration_mode.model()
            || selected_effort != self.stored_collaboration_mode.reasoning_effort()
    }
