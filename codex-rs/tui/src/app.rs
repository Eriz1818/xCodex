...
fn session_summary(
    ...
    let resume_command = codex_core::util::resume_command(thread_name.as_deref(), thread_id)
        .map(|cmd| cmd.replacen("codex ", "xcodex ", 1));
    Some(SessionSummary {
        usage_line,
        resume_command,
    })
                    ...
                    SessionSelection::Resume(path) => {
                            ...
                            .await
                        {
                            Ok(resumed) => {
                                ...
                                let mut init = self.chatwidget_init_for_forked_or_resumed_thread(
                                    tui,
                                    self.config.clone(),
                                );
                                init.model = Some(resumed_model.clone());
                ...
                if let Some(path) = self.chat_widget.rollout_path() {
                        ...
                        .await
                    {
                        Ok(forked) => {
                            ...
                            init.model = Some(forked.session_configured.model.clone());
                            self.chat_widget = ChatWidget::new_from_existing(
                                init,
                                forked.thread,
