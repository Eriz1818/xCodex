...
pub(crate) fn builtins_for_input(flags: BuiltinCommandFlags) -> Vec<(&'static str, SlashCommand)> {
    // `/settings` is the xcodex composer settings menu. Upstream also uses this flag for
    // realtime audio-device selection, but that must not gate xcodex settings visibility.
    let _audio_device_selection_enabled = flags.audio_device_selection_enabled;
