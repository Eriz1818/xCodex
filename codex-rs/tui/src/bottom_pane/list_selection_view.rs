...
mod tests {
    ...
    fn make_selection_view(subtitle: Option<&str>) -> ListSelectionView {
        make_selection_view_with_mode(subtitle, ColumnWidthMode::AutoVisible)
    }

    fn make_selection_view_with_mode(
        subtitle: Option<&str>,
        col_width_mode: ColumnWidthMode,
    ) -> ListSelectionView {
    ...
    #[test]
    fn popup_surface_matches_shared_background_in_all_column_width_modes() {
        for mode in [
            ColumnWidthMode::AutoVisible,
            ColumnWidthMode::AutoAllRows,
            ColumnWidthMode::Fixed,
        ] {
            let view = make_selection_view_with_mode(None, mode);
            assert_popup_surface_bg(Rect::new(0, 0, 48, view.desired_height(48)), |area, buf| {
                view.render(area, buf);
            });
        }
