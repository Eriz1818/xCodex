...
impl TextArea {
    ...
    fn render_lines(
        ...
        let mode_style = if self.use_transcript_style {
            crate::theme::transcript_style()
        } else {
            user_message_style().patch(crate::theme::composer_style())
        };
        let effective_base_style = base_style.patch(mode_style);
        for (row, idx) in range.enumerate() {
            let r = &lines[idx];
            let y = area.y + row as u16;
            let line_range = r.start..r.end - 1;
            buf.set_style(Rect::new(area.x, y, area.width, 1), effective_base_style);
            ...
            for elem in &self.elements {
                ...
                let style = effective_base_style.patch(crate::theme::accent_style());
