...
impl BottomPane {
    ...
    pub fn handle_key_event(&mut self, key_event: KeyEvent) -> InputResult {
        ...
        if !self.view_stack.is_empty() {
            ...
            if ctrl_c_completed {
                ...
                } else if let Some(next_view) = self.view_stack.last()
    ...
    /// quit/interrupt state machine and uses the result to decide what happens next (including
    /// whether to show any quit shortcut hint).
    pub(crate) fn on_ctrl_c(&mut self) -> CancellationEvent {
        if let Some(view) = self.view_stack.last_mut() {
            let event = view.on_ctrl_c();
            if matches!(event, CancellationEvent::Handled) && view.is_complete() {
                self.view_stack.pop();
                self.on_active_view_complete();
            }
            event
    ...
    pub(crate) fn set_footer_status_options(
        ...
        self.composer
            .set_footer_status_options(show_model, show_branch, show_worktree);
        self.request_redraw();
    }

    pub(crate) fn set_footer_model_name(&mut self, footer_model_name: Option<String>) {
        if self.composer.set_footer_model_name(footer_model_name) {
            self.request_redraw();
        }
    }
