...
impl RootIgnore {
    ...
    fn is_ignored_path(&self, path: &Path, is_dir: bool) -> bool {
        ...
        let Some(matcher) = self.matcher.as_ref() else {
            return self.is_ignore_file(path);
        };
...
fn get_file_path<'a>(path: &'a Path, search_directories: &[PathBuf]) -> Option<(usize, &'a str)> {
    let (root_idx, rel_path) = get_root_relative_path(path, search_directories)?;
    rel_path.to_str().map(|p| (root_idx, p))
}

fn get_root_relative_path<'a>(
    path: &'a Path,
...
fn walker_worker(
    ...
    let root_ignores: Vec<RootIgnore> = inner
        .search_directories
        .iter()
        .map(|search_directory| RootIgnore::new(search_directory, inner.ignore_filenames.clone()))
        .collect();
    ...
    walker.run(|| {
        ...
        let root_ignores = root_ignores.clone();
        ...
        Box::new(move |entry| {
            ...
            if let Some((root_idx, _)) = get_root_relative_path(path, &search_directories)
                && root_ignores
                    .get(root_idx)
                    .is_some_and(|root_ignore| root_ignore.is_ignored_path(path, is_dir))
            {
                return if is_dir {
