...
trait ModelsManagerTestCompat {
    fn get_model_offline(model: Option<&str>) -> String;
    fn construct_model_info_offline(
        model_slug: &str,
        config: &Config,
    ) -> codex_protocol::openai_models::ModelInfo;
}

impl ModelsManagerTestCompat for ModelsManager {
    fn get_model_offline(model: Option<&str>) -> String {
        codex_core::test_support::get_model_offline(model)
    }
    ...
    fn construct_model_info_offline(
        ...
        codex_core::test_support::construct_model_info_offline(model_slug, config)
    }
}
...
fn token_info_for(model_slug: &str, config: &Config, usage: &TokenUsage) -> TokenUsageInfo {
    ...
    let full_context_window = model_info.context_window.or(config.model_context_window);
    let percent = model_info.effective_context_window_percent.clamp(1, 100);
    let context_window =
...
async fn status_snapshot_includes_reasoning_details() {
    ...
    config.cwd =
        AbsolutePathBuf::from_absolute_path("/workspace/tests").expect("absolute test cwd");
...
async fn status_snapshot_includes_session_stats_when_available() {
    ...
    config.model_provider_id = "openai".to_string();
    config.cwd =
        AbsolutePathBuf::from_absolute_path(temp_project.path()).expect("absolute test cwd");
