...
pub(crate) fn open_post_plan_prompt(
    ...
    if context_block_reason.is_some() {
        let fix_default_mask = default_mode_mask;
...
fn handle_settings_command(chat: &mut ChatWidget, rest: &str) {
    ...
    if let Some(value) = trimmed.strip_prefix("brainstorm-first ") {
        set_brainstorm_pref(chat, value.trim());
        return;
    }
    ...
    if let Some(value) = trimmed.strip_prefix("flowchart ") {
        set_flowchart_pref(chat, value.trim());
        return;
    }
    ...
    if let Some(value) = trimmed.strip_prefix("track-worktree ") {
        set_plan_track_worktree(chat, value.trim());
        return;
    }
    ...
    if let Some(value) = trimmed.strip_prefix("track-branch ") {
        set_plan_track_branch(chat, value.trim());
        return;
    }
    ...
    if let Some(value) = trimmed.strip_prefix("mismatch-action ") {
        set_plan_mismatch_action(chat, value.trim());
        return;
    }
    ...
    if let Some(value) = trimmed.strip_prefix("naming ") {
        set_plan_naming_strategy(chat, value.trim());
        return;
    }
    ...
    if let Some(value) = trimmed.strip_prefix("model ") {
        set_plan_model(chat, value.trim());
        return;
    }
    ...
    let mut items = vec![
        ...
        SelectionItem {
            ...
            actions: vec![apply_plan_settings_action(
                format!("brainstorm-first {}", next_brainstorm_pref(&brainstorm)),
                true,
            )],
            tab_actions: vec![apply_plan_settings_action(
                format!("brainstorm-first {}", next_brainstorm_pref(&brainstorm)),
                true,
            )],
            dismiss_on_select: true,
        ...
        SelectionItem {
            ...
            actions: vec![apply_plan_settings_action(
                format!("flowchart {}", next_flowchart_pref(&flowchart)),
                true,
            )],
            tab_actions: vec![apply_plan_settings_action(
                format!("flowchart {}", next_flowchart_pref(&flowchart)),
        ...
        SelectionItem {
            ...
            actions: vec![apply_plan_settings_action(
                format!("mismatch-action {}", next_mismatch_action(&mismatch_action)),
                true,
            )],
            tab_actions: vec![apply_plan_settings_action(
                format!("mismatch-action {}", next_mismatch_action(&mismatch_action)),
        ...
        SelectionItem {
            ...
            actions: vec![apply_plan_settings_action(
                format!("naming {}", next_naming_strategy(&naming_strategy)),
                true,
            )],
            tab_actions: vec![apply_plan_settings_action(
                format!("naming {}", next_naming_strategy(&naming_strategy)),
        ...
        SelectionItem {
            name: format!("Plan mode: {mode}"),
            selected_description: Some("Choose default | adr-lite | custom".to_string()),
            actions: vec![open_plan_mode_picker_action()],
            dismiss_on_select: true,
            ..Default::default()
        },
        SelectionItem {
            name: format!("Plan mode model: {plan_model_label}"),
            selected_description: Some("Choose a model override or inherit default".to_string()),
    ...
    chat.show_selection_view(SelectionViewParams {
        ...
        items: vec![
            ...
            SelectionItem {
                ...
                selected_description: Some("Choose the seed template for custom mode".to_string()),
                is_current: mode == PLAN_MODE_CUSTOM,
                actions: vec![open_plan_mode_custom_seed_picker_action()],
                dismiss_on_select: true,
                ..Default::default()
            },
        ],
...
pub(crate) fn open_plan_mode_custom_seed_picker(chat: &mut ChatWidget) {
    let seed = read_custom_seed_mode(chat);
    chat.show_selection_view(SelectionViewParams {
        title: Some("Custom mode seed".to_string()),
        subtitle: Some("Select template seed for custom plan mode".to_string()),
        footer_hint: Some(plan_footer_hint_line(&[
            ("↑/↓", ": select"),
            ("Enter", ": apply"),
            ("Esc", ": back"),
        ...
        items: vec![
            SelectionItem {
                ...
                selected_description: Some("Set mode custom default".to_string()),
                is_current: seed == PLAN_MODE_DEFAULT,
                actions: vec![apply_plan_settings_action(
            ...
            SelectionItem {
                ...
                selected_description: Some("Set mode custom adr-lite".to_string()),
                is_current: seed == PLAN_MODE_ADR_LITE,
                actions: vec![apply_plan_settings_action(
        ...
        initial_selected_idx: Some(if seed == PLAN_MODE_ADR_LITE { 1 } else { 0 }),
        ..Default::default()
    });
}
...
fn show_plan_base_dir(chat: &mut ChatWidget) {
    let base_dir = plan_base_dir(chat);
    let mode = read_plan_mode(chat);
    chat.add_info_message(
        format!(
            "Plan base directory: `{}`\nPlan mode: `{mode}`\nSet with `/plan settings base-dir <path>`.",
            base_dir.display(),
        ),
        None,
    );
...
fn set_brainstorm_pref(chat: &mut ChatWidget, pref: &str) {
...
fn set_flowchart_pref(chat: &mut ChatWidget, pref: &str) {
...
fn set_plan_track_worktree(chat: &mut ChatWidget, raw: &str) {
...
fn set_plan_track_branch(chat: &mut ChatWidget, raw: &str) {
    let Some(value) = normalize_bool_setting(raw) else {
...
fn set_plan_mismatch_action(chat: &mut ChatWidget, raw: &str) {
...
fn set_plan_naming_strategy(chat: &mut ChatWidget, raw: &str) {
...
fn set_plan_model(chat: &mut ChatWidget, raw: &str) {
...
fn set_plan_mode(chat: &mut ChatWidget, raw_mode: &str) {
...
fn open_plan_mode_custom_seed_picker_action() -> SelectionAction {
    Box::new(|sender| sender.send(AppEvent::OpenPlanModeCustomSeedPickerView))
}
...
impl PlanBaseDirEditorView {
    fn new(
        ...
        composer.set_footer_hint_override(Some(vec![
            ("↑/↓".to_string(), "browse @results".to_string()),
            ("Tab".to_string(), "choose @result".to_string()),
            ("Enter".to_string(), "apply".to_string()),
            ("Esc".to_string(), "cancel".to_string()),
        ]));
        composer.set_text_content(initial_value);
        Self {
            composer,
            app_event_tx,
            complete: false,
...
impl BottomPaneView for PlanBaseDirEditorView {
    ...
    fn on_ctrl_c(&mut self) -> CancellationEvent {
        self.app_event_tx.send(AppEvent::OpenPlanSettingsView);
        self.complete = true;
...
impl Renderable for PlanBaseDirEditorView {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        let subtitle_y = area.y.saturating_add(1);
    ...
    fn desired_height(&self, width: u16) -> u16 {
        self.composer.desired_height(width).saturating_add(2)
    }
