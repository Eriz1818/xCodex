...
async fn mcp_slash_command_timeout_persists_and_retries() {
    ...
    chat.submit_user_message(UserMessage::from("/mcp timeout alpha 30".to_string()));

    let persisted = take_persist_mcp_startup_timeout(&mut rx);
    assert_eq!(persisted, vec![("alpha".to_string(), 30)]);

    match op_rx.try_recv() {
        Ok(Op::McpSetStartupTimeout {
            server,
            startup_timeout_sec,
        }) => {
            assert_eq!(server, "alpha");
            assert_eq!(startup_timeout_sec, 30);
        }
        other => panic!("expected McpSetStartupTimeout op, got {other:?}"),
    }
...
fn take_persist_mcp_startup_timeout(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<AppEvent>,
) -> Vec<(String, u64)> {
    let mut out = Vec::new();
    while let Ok(ev) = rx.try_recv() {
        if let AppEvent::PersistMcpStartupTimeout {
            server,
            startup_timeout_sec,
        } = ev
        {
            out.push((server, startup_timeout_sec));
        }
    }
    out
}
